...
This package expresses **AI-OS Fluid Interface Design** as a typed, task-adaptive agent harness. Its concrete capability remains: Use when building or reviewing AI-OS, Hermes, dashboard, desktop, phone/PWA, chat, sheet, drawer, gesture, motion, or streaming-state interfaces. Apply fluid, responsive, interruptible behavior with accessibility and deterministic verification. The harness contract makes memory, planning, action, capability selection, failure handling, and verification explicit.
...
| Module | Strategy | Responsibility in AI-OS Fluid Interface Design |
| --- | --- | --- |
| Memory | `artifact_revision_history` | Retain user intent stream plus decisions, observations, and verification evidence needed to resume safely. |
| Planning | `iterative_composition_plan` | Transform interruptible state model into the ordered, bounded stages defined by this skill. |
...
| Action | `scope_plan_execute_verify_receipt` | Advance the workflow through responsive surfaces while preserving stop conditions and user authority. |
